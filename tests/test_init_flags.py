...
def test_gitignore_keeps_untouched_entries_and_custom_lines(ai_repo):
    ...
    # `.env` was present, so it must not be re-added: 4 lines were (marker,
    # runtime glob, lock exception, blank separator).
    assert reported_count(ignore_line(res)) == 4, ignore_line(res)

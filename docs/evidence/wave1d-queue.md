...
| Q14 | the runtime guard's own back edge | `path coverage` refuses an anchor that has left a live record's declared base behind, but a runtime record that declares NO base bounds nothing — unlike a release record, where a missing base is itself a FAIL. Deliberate and asymmetric: runtime records predate the field, an accepted one cannot be edited to add a line it never carried, and refusing them all would be answered by rewriting approved records. Wave 1c's record on this tree is exactly that case: accepted, live, baseless. | left open by decision, named in `reference.md` and `templates/AUTHORIZATION.md` — the next stage writes the line; the walk will not invent it |

...
| Q14 | the runtime guard's reach, and its three escapes | `path coverage` refuses an anchor that has left a live record's declared base behind. Three ways that answer does not fire, found by a fresh reviewer escaping the tree at `d3c3165` and re-measured here: a record with `status: closed` is exempt, so closing a live stage and narrowing the anchor past its commits is one edit and reads `[PASS] path coverage: 0 protected touches covered`; a runtime record that declares NO base bounds nothing, unlike a release record where a missing base is itself a FAIL; and clearing `governance.window_start_commit` to `""` yields `SKIP(no-window: unset)` with no FAILED line, which predates this wave. | left open by decision for the first two (an accepted record cannot be retro-fitted, and `status` is a self-report the walk believes — the reviewer, not the field, is the control); the third is the never-migrated shape and is named as such. All three are in `reference.md`, `templates/AUTHORIZATION.md` and the wave 1d `CHANGELOG.md` entry. Wave 1c's own record is the baseless case on this tree. |

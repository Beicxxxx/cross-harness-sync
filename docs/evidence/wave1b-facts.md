...
| D7 | re-authoring the five un-pushed stage commits, after the user asked that this project commit as `Beicxxxx` | `git filter-branch --env-filter` over `main..v2.1-dogfood-10d`, then `git rev-parse pre-identity-rewrite-2026-09-22^{tree} HEAD^{tree}` → both `c7d616e3f45f51fceedf38726ab5afe872e0210f`: no file content moved, only identity. `ddcf5f9`→`20242cb`, `b32c818`→`56365c3`, `fed378d`→`1bc7e8e`, `869d5e8`→`d10c34c`, `062fa56`→`ecc17ad`; every hash pointer in this section was repinned to match. The pre-rewrite tip survives as `pre-identity-rewrite-2026-09-22`. What this does NOT fix: 78 commits already on `origin/main` still carry the school address, and rewriting published history is out of scope |
